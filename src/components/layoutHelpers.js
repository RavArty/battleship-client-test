...
export const entityIndices2 = (entity) => {
	let indices = [];
	for (let i = 0; i < entity.length; i++) {
		const position = entity.orientation === "vertical" ? coordsToIndex({ y: entity.position.y + i, x: entity.position.x }) : coordsToIndex({ y: entity.position.y, x: entity.position.x + i });
		indices.push(position);
	}
...
export const isWithinBounds = (entity) => {
	return (entity.orientation === "vertical" && entity.position.y + entity.length <= BOARD_ROWS) || (entity.orientation === "horizontal" && entity.position.x + entity.length <= BOARD_COLUMNS);
};
...
export const calculateOverhang = (entity) => Math.max(entity.orientation === "vertical" ? entity.position.y + entity.length - BOARD_ROWS : entity.position.x + entity.length - BOARD_COLUMNS, 0);

// Checks if the ship you're trying to place is within bounds and the space is free. Both need to return true
export const canBePlaced = (entity, layout) => isWithinBounds(entity) && isPlaceFree(entity, layout);
...
export const getNeighbors = (coords) => {
	...
	// coords.y === 0;
	if (firstRow) {
		neighbors.push({ x: coords.x + 1, y: coords.y }, { x: coords.x - 1, y: coords.y }, { x: coords.x, y: coords.y + 1 });
	}
	...
	let filteredResult = [...new Set(neighbors.map((coords) => coordsToIndex(coords)).filter((number) => number >= 0 && number < BOARD))];
